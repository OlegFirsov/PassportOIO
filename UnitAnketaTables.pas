...
procedure TFormAnketaTables.CMDialogKey(var Msg:TWMKey);//переход между ячейками по Enter или Tab
begin
...
begin
  ...
  PageControl1.ActivePageIndex := 0;//открываем анкету
end;
...
//ячейки, где один знак после дес. запятой
      ...
      begin
        if not (Value.EndsWith(',')) then
          begin
            ...
            //не перескакиваем автоматически
...
begin
 ...
 //[col,row]
 sgAnketa.Cells[0,0] := 'Наименование организации по регистрации (или ФИО физического лица)';
 sgAnketa.Cells[1,0] := hoz;
 sgAnketa.Cells[0,1] := 'Юридический адрес:';
 sgAnketa.Cells[1,1] := '******************';
 sgAnketa.Rows[2].Text := 'почтовый индекс:';
 sgAnketa.Rows[3].Text := 'Республика (край, область)';
 sgAnketa.Cells[1,3] := region;
 sgAnketa.Rows[4].Text := 'Район';
 sgAnketa.Cells[1,4] := rajon;
 sgAnketa.Rows[5].Text := 'Поселок, п/о';
 sgAnketa.Rows[6].Text := 'Телефон:';
 sgAnketa.Rows[7].Text := 'Факс:';
 sgAnketa.Rows[8].Text := 'Адрес электронной почты';
 sgAnketa.Rows[9].Text := 'Расчетный счет, номер';
 sgAnketa.Rows[10].Text := 'Кор. счет';
 sgAnketa.Rows[11].Text := 'ИНН';
 sgAnketa.Rows[12].Text := 'КПП';
 sgAnketa.Rows[13].Text := 'Банк, наименование';
 sgAnketa.Rows[14].Text := 'Город';
 sgAnketa.Rows[15].Text := 'Общая численность работающих:';
 sgAnketa.Rows[16].Text := 'в том числе специалистов';
 sgAnketa.Rows[17].Text := 'ФИО руководителя';
 sgAnketa.Rows[18].Text := 'Имеется всего сельскохозяйственных предприятий';
 sgAnketa.Rows[19].Text := 'Имеется зоотехников, всего чел.:';
 sgAnketa.Rows[20].Text := '% обеспеченности';
 sgAnketa.Rows[21].Text := 'в т.ч. зоотехников-селекционеров, чел.:';
 sgAnketa.Rows[22].Text := '% обеспеченности';
 sgAnketa.Rows[23].Text := 'Техников по племенному учету, чел.:';
 sgAnketa.Rows[24].Text := '% обеспеченности';
 sgAnketa.Rows[25].Text := 'Общая земельная площадь, га';
 sgAnketa.Rows[26].Text := 'в т.ч. пашня, га';
 sgAnketa.Rows[27].Text := 'сенокосы, га.';
 sgAnketa.Row := 2;
 //проверяем это уникальная анкета или уже существующая
 isUnique := DataMdl.isUnique(DataMdl.ADOQueryTables,'anketa',kodregion,kodrajon,kodhoz,year);
 if not isUnique then
 //заполняем существующую анкету
 begin
...
if (ExcelActive=MK_E_UNAVAILABLE) then
  ...
  //[row,col]
  MsExcel.WorkSheets[NameSheet].Cells[3,1].Value := 'осеменению с/х животных за ' + year + ' год';
...
begin
  ...
  if DataMdl.InsertIntoAnketa(DataMdl.ADOQueryAnketa,kodregion,kodrajon,kodhoz,
    ...
    sgAnketa.Cells[1,24],sgAnketa.Cells[1,25],sgAnketa.Cells[1,26],sgAnketa.Cells[1,27],year) > 0 then
      Application.MessageBox('Анкета добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Анкета НЕ добавлена', 'Информация', MB_OK);
  end
    ...
    begin
      if DataMdl.UpdateAnketa(DataMdl.ADOQueryAnketa,kodregion,kodrajon,kodhoz,
        ...
        sgAnketa.Cells[1,24],sgAnketa.Cells[1,25],sgAnketa.Cells[1,26],sgAnketa.Cells[1,27],year,id) > 0 then
          Application.MessageBox('Анкета изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Анкета НЕ изменена', 'Информация', MB_OK);
    end;
  end
  else
    Application.MessageBox('Должны быть заполнены все поля.', 'Информация', MB_OK);
...
begin
 ...
 sgTabl1.Cells[0,0] := '                    Показатели';
 sgTabl1.Cells[1,0] := '   КРС';
 sgTabl1.Cells[2,0] := '   в т.ч.коров';
 sgTabl1.Cells[3,0] := '   Свиней';
 sgTabl1.Cells[4,0] := '   Овец';
 sgTabl1.Cells[0,1] := 'Наличие поголовья в с/х предприятиях, тыс.гол.';
 sgTabl1.Cells[0,2] := 'во всех категориях хозяйств, тыс.гол.';
 sgTabl1.Cells[0,3] := 'Выход приплода на 100 маток';
 sgTabl1.Cells[0,4] := 'Случено и искусственно осеменено, тыс.гол.';
 sgTabl1.Cells[0,5] := 'в т.ч. искусственно, тыс.гол.';
 sgTabl1.Cells[0,6] := 'Охват искусственным осеменением, %';
 sgTabl1.Cells[0,7] := 'Удельный вес искусственного осеменения, %';
 sgTabl1.Cells[0,8] := 'Наличие:';
 sgTabl1.Cells[0,9] := 'племзаводов';
 sgTabl1.Cells[0,10] := 'племрепродукторов';
 sgTabl1.Cells[0,11] := 'Удой коров по всем хозяйствам региона, по данным статотчетности, кг';
 sgTabl1.Cells[0,12] := 'Удой коров по данным бонитировки, все категории хозяйств, кг';
 sgTabl1.Cells[0,13] := 'в т.ч. племзаводы, кг';
 sgTabl1.Cells[0,14] := 'племрепродукторы, кг';
 sgTabl1.Cells[0,15] := 'Средний возраст телок при первой случке, мес.';
 sgTabl1.Cells[0,16] := 'Ввод первотелок в основное стадо, %';
 sgTabl1.Cells[0,17] := 'Выбраковка коров, %';
 sgTabl1.SetFocus;
//проверяем это уникальная tabl1 или уже существующая
...
begin
//перемещаемся по enter, минуя закрашенные ячейки
  if (ord(Key) = VK_RETURN) then
...
var
  posComma: integer;
begin
//эти ячейки не редактируются
    if (ARow = 8) then
      sgTabl1.Cells[ACol,ARow] := '';
    if (ARow > 10) and (ACol > 2) then
      sgTabl1.Cells[ACol,ARow] := '';
//зачем проверять пустую ячейку?
...
begin
    ...
    if ((ARow = 6) or (ARow = 7) or (ARow = 16) or (ARow = 17))
      ...
      begin
        ...
        Application.MessageBox('% не может быть больше 100.', 'Информация', MB_OK);
        exit;
      end;
//Контроль ввода 1 знак после запятой
...
//Контроль ввода 2 знака после запятой
if (ARow = 11) then
    begin
    posComma := Value.IndexOf(',');
    if  (posComma = 0) then
...
//в этих ячейках одинаковые значения, так зачем вводить дважды
...
if DataMdl.InsertIntoTabl1(DataMdl.ADOQueryAnketa,
    ...
    sgTabl1.Cells[1,15],sgTabl1.Cells[1,16],sgTabl1.Cells[1,17],kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl1 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl1 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
    ...
    sgTabl1.Cells[1,15],sgTabl1.Cells[1,16],sgTabl1.Cells[1,17],id) > 0 then
        Application.MessageBox('Tabl1 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl1 НЕ изменена', 'Информация', MB_OK);
    end;
end;
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу1?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
        if (not isUnique(ADOQuerySprSost,'tabl1',kodregion,kodrajon,kodhoz,year)) then
          begin
            ...
            Application.MessageBox('А нечего удалять!', 'Информация', MB_OK);
      end;
end;
...
if DataMdl.InsertIntoTabl2(DataMdl.ADOQueryAnketa,
    ...
    kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl2 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl2 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
    ...
    sgTabl2.Cells[1,10],sgTabl2.Cells[2,10],sgTabl2.Cells[3,10],id) > 0 then
        Application.MessageBox('Tabl2 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl2 НЕ изменена', 'Информация', MB_OK);
    end;
end;
...
if (ExcelActive=MK_E_UNAVAILABLE) then
  ...
  MsExcel.WorkSheets[NameSheet].Cells[3,1].Value := '   Код региона ' + kodregion +
    '      Код района ' + kodrajon + '      Код хозяйства ' + kodhoz + '              Год ';
...
begin
 ...
 sgTabl2.Cells[0,0] := '                    Показатели';
 sgTabl2.Cells[1,0] := '   Быков';
 sgTabl2.Cells[2,0] := '   Хряков';
 sgTabl2.Cells[3,0] := '   Баранов';
 sgTabl2.Cells[0,1] := 'Имеется всего, гол.';
 sgTabl2.Cells[0,2] := 'в т.ч. в ОИО, гол.';
 sgTabl2.Cells[0,3] := 'из них ремонтных*';
 sgTabl2.Cells[0,4] := 'Оценено по качеству потомства за год, гол.';
 sgTabl2.Cells[0,5] := 'Выявлено с категорией "Улучшатель", гол.';
 sgTabl2.Cells[0,6] := '%';
 sgTabl2.Cells[0,7] := 'Из общего числа производителей имеют 1 кл и ниже, гол.';
 sgTabl2.Cells[0,8] := '%';
 sgTabl2.Cells[0,9] := 'в т.ч. в ОИО, гол.';
 sgTabl2.Cells[0,10] := '%';
 sgTabl2.SetFocus;
//проверяем это уникальная tabl2 или уже существующая
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу2?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl3.Cells[0,0] := '      5001-6000 кг';
 sgTabl3.Cells[1,0] := '      6001-7000 кг';
 sgTabl3.Cells[2,0] := '      7001-8000 кг';
 sgTabl3.Cells[3,0] := '      8001-9000 кг';
 sgTabl3.Cells[4,0] := '      9001-10 000 кг';
 sgTabl3.Cells[5,0] := '      Более 10 001 кг';
 sgTabl3.SetFocus;
//проверяем это уникальная tabl1 или уже существующая
 isUnique := DataMdl.isUnique(DataMdl.ADOQueryTables,'Tabl3',kodregion,kodrajon,kodhoz,year);
 if not isUnique then
 //заполняем существующую tabl3
 begin
  with DataMdl.ADOQueryTables do
    begin
      k := 1;
...
if DataMdl.InsertIntoTabl3(DataMdl.ADOQueryAnketa,sgTabl3.Cells[0,1],sgTabl3.Cells[1,1],
    ...
    kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl3 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl3 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
      if DataMdl.UpdateTabl3(DataMdl.ADOQueryAnketa,sgTabl3.Cells[0,1],
        ...
        Application.MessageBox('Tabl3 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl3 НЕ изменена', 'Информация', MB_OK);
    end;
end;
...
if (ExcelActive=MK_E_UNAVAILABLE) then
  ...
  MsExcel.WorkSheets[NameSheet].Activate;
  // excel[row,col] stringgrid[col,row]
  MsExcel.WorkSheets[NameSheet].Cells[5,1].Value := '   Код региона ' + kodregion +
    '      Код района ' + kodrajon + '      Код хозяйства ' + kodhoz + '              Год '
    + year + '      Таблица 3';
      for j := 1 to sgTabl3.RowCount -1 do
        for i := 0 to sgTabl3.ColCount -1 do
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу3?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl4.Cells[0,0] := '                    Показатели';
 sgTabl4.Cells[1,0] := '   Всего, голов';
 sgTabl4.Cells[2,0] := '   % несоответствия';
 sgTabl4.Cells[0,1] := 'Крупный рогатый скот';
 sgTabl4.Cells[0,2] := 'в т.ч. быков-производителей';
 sgTabl4.Cells[0,3] := 'коров';
 sgTabl4.Cells[0,4] := 'телок';
 sgTabl4.Cells[0,5] := 'бычков';
 sgTabl4.Cells[0,6] := 'Стоимость исследования 1 гол., руб.';
 sgTabl4.Cells[2,6] := ' *******************';
 sgTabl4.SetFocus;
//проверяем это уникальная tabl1 или уже существующая
 isUnique := DataMdl.isUnique(DataMdl.ADOQueryTables,'Tabl4',kodregion,kodrajon,kodhoz,year);
 if not isUnique then
 //заполняем существующую tabl4
 begin
  with DataMdl.ADOQueryTables do
    begin
      k := 1;
...
if DataMdl.InsertIntoTabl4(DataMdl.ADOQueryAnketa,sgTabl4.Cells[1,1],sgTabl4.Cells[2,1],
    ...
    sgTabl4.Cells[1,6],kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl4 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl4 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
      if DataMdl.UpdateTabl4(DataMdl.ADOQueryAnketa,sgTabl4.Cells[1,1],sgTabl4.Cells[2,1],
        ...
        Application.MessageBox('Tabl4 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl4 НЕ изменена', 'Информация', MB_OK);
    end;
end;
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу4?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl5.Cells[0,0] := '                    Показатели';
 sgTabl5.Cells[1,0] := '   Количество';
 sgTabl5.Cells[0,1] := 'Всего лабораторий:';
 sgTabl5.Cells[0,2] := 'обслуживается хозяйств';
 sgTabl5.Cells[0,3] := 'в них коров, тыс.';
 sgTabl5.Cells[0,4] := 'имеется приборов';
 sgTabl5.Cells[0,5] := 'марка приборов';
 sgTabl5.Cells[0,6] := 'Стоимость одной пробы:';
 sgTabl5.Cells[0,7] := 'жир, руб.';
 sgTabl5.Cells[0,8] := 'белок, руб.';
 sgTabl5.Cells[0,9] := 'жир и белок, руб.';
 sgTabl5.Cells[0,10] := 'соматические клетки, руб.';
 sgTabl5.Cells[1,6] := '******************';
 sgTabl5.SetFocus;
//проверяем это уникальная tabl1 или уже существующая
...
if DataMdl.InsertIntoTabl5(DataMdl.ADOQueryAnketa,sgTabl5.Cells[1,1],sgTabl5.Cells[1,2],
    ...
    kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl5 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl5 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
      if DataMdl.UpdateTabl5(DataMdl.ADOQueryAnketa,sgTabl5.Cells[1,1],sgTabl5.Cells[1,2],
        ...
        Application.MessageBox('Tabl5 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl5 НЕ изменена', 'Информация', MB_OK);
    end;
end;
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу5?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl6.Cells[0,0] := '  Тыс.голов';
 sgTabl6.Cells[0,1] := '  % ';
 sgTabl6.SetFocus;
//проверяем это уникальная tabl6 или уже существующая
...
if DataMdl.InsertIntoTabl6(DataMdl.ADOQueryAnketa,sgTabl6.Cells[1,0],sgTabl6.Cells[2,0],
    ...
    sgTabl6.Cells[5,1],sgTabl6.Cells[6,1],kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl6 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl6 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
    ...
    sgTabl6.Cells[5,1],sgTabl6.Cells[6,1],id) > 0 then
        Application.MessageBox('Tabl6 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl6 НЕ изменена', 'Информация', MB_OK);
    end;
end;

procedure TFormAnketaTables.btnExcelTabll6Click(Sender: TObject);
begin
  ExportToExcelTabl6('табл6');
end;
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу6?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl7.Cells[0,0] := '                    Показатели';
 sgTabl7.Cells[1,0] := '   Количество';
 //sgTabl7.Cells[0,1] := 'Имеется на племпредприятии быков с наличием спермы, проверенных по качеству потомства, гол.';
 sgTabl7.Cells[0,2] := '%';
 sgTabl7.Cells[0,3] := 'Проверялось всего гол.';
 sgTabl7.Cells[0,4] := 'в т.ч. живых, гол.';
 sgTabl7.Cells[0,5] := 'Количество хоз-в, в которых проверяются быки';
 sgTabl7.Cells[0,6] := 'Осеменено коров и телок для проверки быков, гол.';
 sgTabl7.SetFocus;
//проверяем это уникальная tabl7 или уже существующая
...
//многострочный
  ...
  begin
    ...
    s := 'Имеется на племпредприятии быков с наличием спермы, проверенных по качеству потомства, гол.';
    Flag := DT_LEFT;
    ...
    if H > sgTabl7.RowHeights[ARow] then
      sgTabl7.RowHeights[ARow] := H;  //увеличиваем
  end;
end;
...
if DataMdl.InsertIntoTabl7(DataMdl.ADOQueryAnketa,sgTabl7.Cells[1,1],sgTabl7.Cells[1,2],
    ...
    kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl7 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl7 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
      if DataMdl.UpdateTabl7(DataMdl.ADOQueryAnketa,sgTabl7.Cells[1,1],sgTabl7.Cells[1,2],
        ...
        Application.MessageBox('Tabl7 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl7 НЕ изменена', 'Информация', MB_OK);
    end;
end;
...
if (ExcelActive=MK_E_UNAVAILABLE) then
  ...
  MsExcel.WorkSheets[NameSheet].Activate;
  // excel[row,col] stringgrid[col,row]
  MsExcel.WorkSheets[NameSheet].Cells[4,1].Value := '   Код региона ' + kodregion +
    '      Код района ' + kodrajon + '      Код хозяйства ' + kodhoz + '              Год '
    + year + '      Таблица 7';
      for j := 1 to sgTabl7.RowCount -1 do
        for i := 1 to sgTabl7.ColCount -1 do
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу7?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl8.Cells[0,0] := '                    Показатели';
 sgTabl8.Cells[1,0] := '   Количество';
 sgTabl8.Cells[0,1] := 'Имеется подконтрольного поголовья коров, тыс. гол.';
 sgTabl8.Cells[0,2] := 'отобрано в племядро, тыс.гол.';
 sgTabl8.Cells[0,3] := 'выделено потенциальных быкородящих коров, гол.';
// sgTabl8.Cells[0,4] := 'содержится коров-первотелок в специализированных дворах общей численностью, гол.';
 sgTabl8.Cells[0,6] := 'Имеется дочерей проверяемых быков на контроле:';
 sgTabl8.Cells[1,6] := '******************';
 sgTabl8.Cells[0,7] := 'коров-первотелок, гол.';
 sgTabl8.Cells[0,8] := 'осемененных телок, гол.';
 sgTabl8.Cells[0,9] := 'не осемененных телок в возрасте старше 1 года, гол.';
 sgTabl8.Cells[0,10] := 'не осемененных телок в возрасте до 1 года, гол.';
 sgTabl8.SetFocus;
//проверяем это уникальная tabl8 или уже существующая
...
begin
  if ((ARow = 6) and (ACol = 1)) then
    ...
    begin
      ...
      //s := sgTabl7.Cells[ACol,ARow];
      s := 'содержится коров-первотелок в специализированных дворах общей численностью, гол.';
      Flag := DT_LEFT;
      //Flag := Flag or DT_CALCRECT;
      Flag := Flag or DT_WORDBREAK;
      Inc(Rect.Left,3);
      ...
      if H > sgTabl8.RowHeights[ARow] then
        sgTabl8.RowHeights[ARow] := H;  //увеличиваем
    end;
    if ((ARow = 5) and (ACol = 0)) then
    begin
      sgTabl8.Canvas.FillRect(Rect);
      //s := sgTabl7.Cells[ACol,ARow];
      s := 'содержится коров-первотелок в специализированных группах численностью, гол.';
      Flag := DT_LEFT;
      //Flag := Flag or DT_CALCRECT;
      Flag := Flag or DT_WORDBREAK;
      Inc(Rect.Left,3);
...
if DataMdl.InsertIntoTabl8(DataMdl.ADOQueryAnketa,sgTabl8.Cells[1,1],sgTabl8.Cells[1,2],
    ...
    sgTabl8.Cells[1,9],sgTabl8.Cells[1,10],kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl8 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl8 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
      if DataMdl.UpdateTabl8(DataMdl.ADOQueryAnketa,sgTabl8.Cells[1,1],sgTabl8.Cells[1,2],
        ...
        Application.MessageBox('Tabl8 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl8 НЕ изменена', 'Информация', MB_OK);
    end;
end;
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу8?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl9.Cells[0,0] := '                    Показатели';
 sgTabl9.Cells[1,0] := '   Количество';
 sgTabl9.Cells[0,1] := 'Охват искусственным осеменением в личных и фермерских хозяйствах коров и телок, %';
 sgTabl9.Cells[0,2] := 'Имеется пунктов искусственного осеменения крупного рогатого скота';
 sgTabl9.Cells[0,3] := 'Аттестовано пунктов искусственного осеменения';
 sgTabl9.Cells[0,4] := 'Имеется техников по искусственному осеменению крупного рогатого скота';
 sgTabl9.Cells[0,5] := 'в т.ч. техников по искусственному осеменению  в штате племпредприятия';
 sgTabl9.Cells[1,6] := '**********';
 sgTabl9.Cells[0,6] := 'Искусственно осеменено коров и телок спермой:';
 sgTabl9.Cells[0,7] := 'замороженной в соломинках, тыс.гол.';
 sgTabl9.Cells[0,8] := 'в облицованных гранулах, тыс.гол.';
 sgTabl9.Cells[0,9] := 'в открытых гранулах, тыс.гол.';
 sgTabl9.Cells[1,10] := '**********';
 sgTabl9.Cells[0,10] := 'Искусственно осеменено коров и телок:';
 sgTabl9.Cells[0,11] := 'ректо-цервикальным способом, тыс.гол.';
 sgTabl9.Cells[0,12] := 'мано-цервикальным способом, тыс.гол.';
 sgTabl9.Cells[0,13] := 'визо-цервикальным, тыс.гол.';
 sgTabl9.Cells[1,14] := '**********';
 sgTabl9.Cells[0,14] := 'Заморожено спермы быков:';
 sgTabl9.Cells[0,15] := 'в соломинках, тыс.доз';
 sgTabl9.Cells[0,16] := 'в облицованных гранулах, тыс.доз';
 sgTabl9.Cells[0,17] := 'в открытых гранулах, тыс.доз';
 //sgTabl9.Cells[1,18] := '**********';
 //sgTabl9.Cells[0,18] := 'Подготовлено:';
 sgTabl9.Cells[0,18] := 'Подготовлено операторов по воспроизводству';
 //sgTabl9.Cells[0,20] := 'Повышена квалификция:';
 //sgTabl9.Cells[1,20] := '**********';
 sgTabl9.Cells[0,19] := 'Повышена квалификция операторов по воспроизводству';
 sgTabl9.Cells[1,20] := '**********';
 sgTabl9.Cells[0,20] := 'Проведено конкурсов на лучшего по профессии:';
 sgTabl9.Cells[0,21] := 'районных';
 sgTabl9.Cells[0,22] := 'зональных';
 sgTabl9.Cells[0,23] := 'областных (краевых, республиканских)';
 sgTabl9.SetFocus;
//проверяем это уникальная Tabl9 или уже существующая
 isUnique := DataMdl.isUnique(DataMdl.ADOQueryTables,'Tabl9',kodregion,kodrajon,kodhoz,year);
 if not isUnique then
 //заполняем существующую tabl9
 begin
  with DataMdl.ADOQueryTables do
    begin
      k := 1;
...
if DataMdl.InsertIntoTabl9(DataMdl.ADOQueryAnketa,sgTabl9.Cells[1,1],sgTabl9.Cells[1,2],
    ...
    sgTabl9.Cells[1,21],sgTabl9.Cells[1,22],sgTabl9.Cells[1,23],kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl9 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl9 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
      if DataMdl.UpdateTabl9(DataMdl.ADOQueryAnketa,sgTabl9.Cells[1,1],sgTabl9.Cells[1,2],
        ...
        Application.MessageBox('Tabl9 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl9 НЕ изменена', 'Информация', MB_OK);
    end;
end;
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу9?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl9a.SetFocus;
//проверяем это уникальная Tabl9a или уже существующая
...
if DataMdl.InsertIntoTabl9a(DataMdl.ADOQueryAnketa,sgTabl9a.Cells[0,1],sgTabl9a.Cells[1,1],
    ...
    sgTabl9a.Cells[7,1],kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl9a добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl9a НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
      if DataMdl.UpdateTabl9a(DataMdl.ADOQueryAnketa,sgTabl9a.Cells[0,1],
        ...
        Application.MessageBox('Tabl9a изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl9a НЕ изменена', 'Информация', MB_OK);
    end;
end;

procedure TFormAnketaTables.btnExcelTabl9aClick(Sender: TObject);
begin
  ExportToExcelTabl9a('табл9а');
end;
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу9a?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl10.Cells[0,0] := '                    Показатели';
 sgTabl10.Cells[1,0] := '   Быков';
 sgTabl10.Cells[2,0] := '   Хряков';
 sgTabl10.Cells[3,0] := '   Баранов';
 sgTabl10.Cells[0,1] := 'Среднегодовое поголовье производителей, гол.';
 sgTabl10.Cells[0,2] := 'Заморожено спермы, тыс. доз';
 sgTabl10.Cells[0,3] := 'Реализовано спермы, всего, тыс. доз';
 sgTabl10.Cells[0,4] := 'в т.ч. в другие регионы, тыс. доз';
 sgTabl10.Cells[0,5] := 'Израсходовано спермы на одно плодотворное осеменение, доз';
 sgTabl10.Cells[0,6] := 'Себестоимость 1 полученной дозы спермы, руб.';
 sgTabl10.Cells[0,7] := 'Затраты на хранение 1 дозы спермы, руб.';
 sgTabl10.Cells[0,8] := 'Цена реализации 1 дозы спермы на конец года, руб.';
 sgTabl10.Cells[0,9] := 'Субсидия на 1 дозу спермы из местного бюджета, руб.';
 sgTabl10.Cells[0,10] := 'Затраты на содержание одного производителя, руб.';
 sgTabl10.Cells[0,11] := 'Расход азота за год, т';
 sgTabl10.Cells[0,12] := 'Стоимость 1 тонны азота. руб.';
 sgTabl10.Cells[0,13] := 'Выбраковка производителей,  гол.';
 sgTabl10.SetFocus;
//проверяем это уникальная Tabl10 или уже существующая
...
if DataMdl.InsertIntoTabl10(DataMdl.ADOQueryAnketa,
    ...
    kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl10 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl10 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
      if DataMdl.UpdateTabl10(DataMdl.ADOQueryAnketa,
        ...
        Application.MessageBox('Tabl10 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl10 НЕ изменена', 'Информация', MB_OK);
    end;
end;
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу10?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
...
begin
 ...
 sgTabl11.Cells[0,0] := '                    Показатели';
 sgTabl11.Cells[1,0] := '   Количество';
 sgTabl11.Cells[0,1] := 'За год получено прибыли, тыс.руб.';
 sgTabl11.Cells[0,2] := 'Дебиторская задолженность, тыс.руб.';
 sgTabl11.Cells[0,3] := 'Кредиторская задолженность, тыс.руб.';
 sgTabl11.Cells[0,4] := 'Среднемесячная заработная плата, руб.';
 sgTabl11.Cells[1,5] := '**********';
 sgTabl11.Cells[0,5] := 'Износ основных средств, %';
 sgTabl11.Cells[0,6] := 'зданий и сооружений';
 sgTabl11.Cells[0,7] := 'машин и оборудования';
 sgTabl11.Cells[1,8] := '**********';
 sgTabl11.Cells[0,8] := 'Фактическая поддержка на племенное дело по ОИО, тыс.руб.:';
 sgTabl11.Cells[0,9] := 'из местного бюджета, тыс.руб.';
 sgTabl11.Cells[0,10] := 'из федерального бюджета, тыс.руб.';
 sgTabl11.Cells[1,11] := '**********';
 sgTabl11.Cells[0,11] := 'Дотация на 1 реализованную дозу семени:';
 sgTabl11.Cells[0,12] := 'из местного бюджета, руб';
 sgTabl11.Cells[0,13] := 'из федерального бюджета, руб.';
 sgTabl11.SetFocus;
//проверяем это уникальная Tabl11 или уже существующая
 isUnique := DataMdl.isUnique(DataMdl.ADOQueryTables,'Tabl11',kodregion,kodrajon,kodhoz,year);
 if not isUnique then
 //заполняем существующую Tabl11
 begin
  with DataMdl.ADOQueryTables do
    begin
      k := 1;
...
begin
  ...
  if DataMdl.InsertIntoTabl11(DataMdl.ADOQueryAnketa,sgTabl11.Cells[1,1],sgTabl11.Cells[1,2],
    ...
    kodregion,kodrajon,kodhoz,year) > 0 then
      Application.MessageBox('Tabl11 добавлена!', 'Информация', MB_OK)
    else
      Application.MessageBox('Ошибка! Tabl11 НЕ добавлена', 'Информация', MB_OK);
  end
  //редактирование существующего
  else
    begin
    ...
    sgTabl11.Cells[1,9],sgTabl11.Cells[1,10],sgTabl11.Cells[1,12],sgTabl11.Cells[1,13],id) > 0 then
        Application.MessageBox('Tabl11 изменена!', 'Информация', MB_OK)
      else
        Application.MessageBox('Ошибка! Tabl11 НЕ изменена', 'Информация', MB_OK);
    end;
end;

procedure TFormAnketaTables.btnExcelTabl11Click(Sender: TObject);
begin
  ExportToExcelTabl11('табл11');
end;
...
var
  i,j : integer;
begin
  if MessageDlg('Удалить таблицу11?', mtConfirmation,[mbOK, mbCancel], 0)=mrOK then
    with DataMdl do
      begin
