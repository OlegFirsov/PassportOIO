...
begin
  ...
  result := ResultCode=200;
    if not result then
      //raise Exception.Create(IntToStr(ResultCode) + ' ' + ResultString)  Не подымаем исключение
      Application.MessageBox('Отсутствует файл обновления.Обратитесь к администратору.', 'Информация', MB_OK)
   else
    begin
      try
    ...
    //raise Exception.Create(rsPropfindError + ' ' + ResultString);  Не подымаем исключение
    Application.MessageBox('Отсутствует связь с сервером обновления.Проверьте Интернет.', 'Информация', MB_OK);
end;
end;
