...
procedure SelectAnketa(ADOQuery:TADOQuery;Where:string);
  ...
  private
    ...
    Fkodh: integer;//код хозяйства
    Fp_ind: string;//почтовый индекс 6 зн.
    Fposel: string;//Поселок, п/о
    Ftel: string;//Телефон
    Ffax: string;//факс
    Femail: string;//Адрес электронной почты
    Frs: string;//Расчетный счет, номер
    Fks: string;//Кор. счет
    Fnalog: string;//ИНН
    Fbik_rs: string;//КПП
    Fbank: string;//Банк, наименование
    Ftown: string;//Город банка
    Fstrength: string;//Общая численность работающих
    Fspec: string;//в том числе специалистов
    Ffam: string;//ФИО руководителя
    Fcolpred: string;//Имеется всего сельскохозяйственных предприятий
    Fcolzootech: string;//Имеется зоотехников, всего чел.
    Fproczootech: string;//% обеспеченности
    Fcolzoosel: string;//в т.ч. зоотехников-селекционеров, чел.
    Fproczoosel: string;//% обеспеченности
    Fcoltechplem: string;//Техников по племенному учету, чел.
    Fproctechplem: string;//% обеспеченности
    Fpl: string;//Общая земельная площадь, га
    Fpash: string;//в т.ч. пашня, га
    Fseno: string;//сенокосы, га.
    Fgod: string;
  public
    { Public declarations }
...
begin
  Fid := id;//счетчик
  Fkodr := kodr;//код региона
  Fkodrn := kodrn;//код района
  Fkodh := kodh;//код хозяйства
  Fp_ind := p_ind;//почтовый индекс 6 зн.
  Fposel := posel;//Поселок, п/о
  Ftel := tel;//Телефон
  Ffax := fax;//факс
  Femail := email;//Адрес электронной почты
  Frs := rs;//Расчетный счет, номер
  Fks := ks;//Кор. счет
  Fnalog := nalog;//ИНН
  Fbik_rs := bik_rs;//КПП
  Fbank := bank;//Банк, наименование
  Ftown := town;//Город банка
  Fstrength := strength;//Общая численность работающих
  Fspec := spec;//в том числе специалистов
  Ffam := fam;//ФИО руководителя
  Fcolpred := colpred;//Имеется всего сельскохозяйственных предприятий
  Fcolzootech := colzootech;//Имеется зоотехников, всего чел.
  Fproczootech := proczootech;//% обеспеченности
  Fcolzoosel := colzoosel;//в т.ч. зоотехников-селекционеров, чел.
  Fproczoosel := proczoosel;//% обеспеченности
  Fcoltechplem := coltechplem;//Техников по племенному учету, чел.
  Fproctechplem := proctechplem;//% обеспеченности
  Fpl := pl;//Общая земельная площадь, га
  Fpash := pash;//в т.ч. пашня, га
  Fseno :=  seno;//сенокосы, га.
  Fgod := year;
end;
...
begin
  ADOQuery.Close;
  ADOQuery.Sql.Clear;
  ADOQuery.Sql.Add('SELECT  anketa.id, spobl.im as [Регион], kodra.im as [Район], kodxoz.im as [Хозяйство],'
    + ' anketa.god as [Год], anketa.fam as [ФИО руководителя], anketa.colpred as [Всего с/х предприятий],'
    + ' anketa.colzootech as [Зоотехников,чел], anketa.proczootech as [% з/т], '
    + ' anketa.colzoosel as [в т/ч зоотехников-селекционеров,чел], anketa.proczoosel as [% з/с],'
    + ' anketa.coltechplem as [Техников по племенному учету,чел], anketa.proctechplem as [% т/пу],'
    + ' anketa.pl as [Общая земельная площадь,га], anketa.pash as [в т/ч пашня,га], anketa.seno as [сенокосы,га]'
